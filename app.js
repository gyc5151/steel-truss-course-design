(() => {
  ...
  function parseFactors(raw) {
    ...
    return {
      gammaG: Number(parts[0]) || 1.2,
      gammaQ: Number(parts[1]) || 1.4
    };
  }
  ...
  function analyzeMembers(model, cases, input) {
    return model.members.map((member, index) => {
      ...
      return {
        ...
        slenderLimit: member.group === "下弦" ? 350 : 150,
  ...
  function buildNodeCards(model, memberRows, input) {
    ...
    return pick.map((item) => {
      ...
      const plate = control <= 100 ? 8 : control <= 200 ? 10 : control <= 350 ? 12 : control <= 500 ? 14 : 16;
      return {
        ...item,
  ...
  function calculate() {
    ...
    Object.assign(state, { input, loads, model, cases, members, sectionRows, nodes, report });
    render();
  }
  ...
  function renderSummary() {
    ...
    const metrics = [
      ...
      ["导出", "Markdown / CSV / SVG", "可打印为 PDF，数据保存在本机浏览器"]
    ];
  ...
  function renderNotices() {
    ...
    const notices = [
      ...
      ["杆件内力来自平面桁架刚度法和内置梯形屋架简图。提交前应对照老师给定屋架几何尺寸及内力系数图复核。", "warn"],
      ["参考实例中的分项系数文字与数值可能不一致。本工具默认 γG=1.2、γQ=1.4；如课堂要求不同，可在左侧直接修改。", "warn"],
  ...
  function makeTrussSvg() {
    ...
    const scaleY = 230 / Math.max(1, maxY);
    ...
    const memberLines = members.map((m) => {
      ...
      return `<line x1="${a.x}" y1="${a.y}" x2="${b.x}" y2="${b.y}" stroke="${color}" stroke-width="${nf(widthLine, 2)}" stroke-linecap="round"><title>${m.id} ${m.group} ${nf(force, 1)} kN</title></line>`;
    }).join("");
    const nodeDots = model.nodes.map((node) => {
      const p = pt(node);
      return `<circle cx="${p.x}" cy="${p.y}" r="3.2" fill="#20292b"><title>${node.id} (${nf(node.x, 2)}, ${nf(node.y, 2)})</title></circle>`;
    }).join("");
    const labels = model.nodes.filter((_, i) => i % Math.max(1, Math.floor(model.n / 6)) === 0).map((node) => {
      const p = pt(node);
      const dy = node.kind === "top" ? -10 : 18;
      return `<text x="${p.x}" y="${p.y + dy}" text-anchor="middle" class="svg-label">${node.id}</text>`;
    }).join("");
    const loadsSvg = model.top.map((nodeIndex, i) => {
      ...
      return `<g>
        <line x1="${p.x}" y1="${p.y - 54}" x2="${p.x}" y2="${p.y - 10}" stroke="#6750a4" stroke-width="1.5" marker-end="url(#arrow)" />
        <text x="${p.x}" y="${p.y - 62}" text-anchor="middle" class="svg-label">${nf(load, 1)}</text>
      </g>`;
    }).join("");
    const dimensionY = baseY + 42;
    return `
      <svg viewBox="0 0 ${width} ${height}" role="img" aria-label="梯形钢屋架计算简图">
        ...
        <text x="${width / 2}" y="58" text-anchor="middle" class="svg-note">红色偏拉、绿色偏压；紫色为组合一节点荷载 kN</text>
        ${loadsSvg}
        ${memberLines}
        ${nodeDots}
        ${labels}
        ...
        <style>
          ...
          .svg-note,.svg-label{font:12px -apple-system,BlinkMacSystemFont,"Segoe UI","Microsoft YaHei",sans-serif;fill:#46534f}
        </style>
      </svg>
    `;
  }
  ...
  function renderSections() {
    ...
    $("#sectionTable").innerHTML = table(["杆件", "类别", "控制内力 kN", "建议截面", "A mm²", "imin mm", "λ", "φ", "N/A N/mm²", "状态"], rows, true);
  }
...
截面初选按双角钢截面库进行。拉杆按 N/A ≤ f 控制，压杆按 N/(φA) ≤ f 并验算长细比。该截面库用于课程设计初选，最终应查教材型钢表复核。
...
节点板厚度初取 ${n.plate} mm，节点板外形按焊缝长度、20 mm 杆端间隙及不小于 15° 的构造要求绘图确定。`).join("\n\n")}

## 7 施工图整理提示

施工图应包括屋架简图、半榀屋架正面图、上下弦平面图、侧面图、剖面图、必要零件详图、材料表、焊缝和螺栓说明、标题栏。屋架简图左半跨标杆件长度，右半跨标最不利内力，并注明起拱度。
